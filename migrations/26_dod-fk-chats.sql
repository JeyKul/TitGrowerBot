-- Titties_of_Day.chat_id was converted from the raw Telegram id to the Chats surrogate id
-- back in migration 10, but (unlike Tits in migration 14) never got the matching FK.
--
-- Without it, a chat merge could delete the losing Chats row and leave this table behind: Tits
...
DELETE FROM Titties_of_Day dod
WHERE NOT EXISTS (SELECT 1 FROM Chats c WHERE c.id = dod.chat_id);

ALTER TABLE Titties_of_Day ADD FOREIGN KEY (chat_id) REFERENCES Chats(id);
