-- no-transaction
-- Speeds up the daily scan for stale tits. Partial on `length > 0` because only positive tits
-- can shrink — matching the WHERE of perform_daily_shrink — which keeps the index small and
-- selective.
--
-- Built CONCURRENTLY so it doesn't lock Tits against grow/pvp writes for the duration of the
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS tits_idx_updated_at ON Tits(updated_at) WHERE length > 0;
