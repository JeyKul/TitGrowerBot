#!/usr/bin/env bash
# Seeds Stale_Tit_Shrinks (and the stale Tits that feed it) so each shrink-paging case can be
...
#
# By default today's shrinks come from the bot actually running: the seeded tits are left stale so
...
#
# --history-only writes today's rows directly instead and leaves the tits freshly grown, so the
# whole history is testable in the inline command with no run and no restart. The tits have to be
...
#
# --clear removes the seeded users, their tits and their shrink rows again. Worth doing when you
# are finished: the seeded tits are ordinary rows, so until then they sit at the top of /top.
#
...
if $CLEAR; then
    TG_CHAT_ID="${1:?$USAGE}"
    echo "--clear: removing this script's users, tits and shrink rows from $TG_CHAT_ID"
    docker exec -i "${POSTGRES_CONTAINER:-titgrowerbot-postgresql}" \
        psql -U "${POSTGRES_USER:-titgrowerbot}" -d "${POSTGRES_DB:-titgrowerbotdb}" \
...
DELETE FROM Users u
    WHERE u.uid >= :uid_base
      AND NOT EXISTS (SELECT 1 FROM Tits d WHERE d.uid = u.uid);
COMMIT;
...
CONTAINER="${POSTGRES_CONTAINER:-titgrowerbot-postgresql}"
DB_USER="${POSTGRES_USER:-titgrowerbot}"
DB_NAME="${POSTGRES_DB:-titgrowerbotdb}"
...
$HISTORY_ONLY && echo "--history-only: seeding today directly, tits left fresh (no run, no notification)"
...
run it in the chat. No restart, and no notification either (the tits are fresh, so a run would
...
and the daily job will shrink those tits immediately and post the notification, adding today to
