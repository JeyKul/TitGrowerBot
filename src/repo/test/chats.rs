...
async fn migrate_chat_id() {
    ...
    let internal_id = chats.upsert_chat(&old_partiality)
        .await.expect("couldn't create a chat");
    tits.create_or_grow(USER_ID, &old_partiality, LengthChange::signed(5))
    ...
    // the row keeps its internal id, so the tit follows the chat to the supergroup
    ...
    let top = tits.get_top(&ChatIdKind::ID(new), Offset::new(0), Limit::literal(1), DaysCount::new(7))
...
async fn merge_moves_dependent_rows() {
    ...
    assert_eq!(chat.tit_of_the_day_days_ago().await, 1);
    assert_eq!(chat.tits_of_the_day_left().await, 0);
...
#[tokio::test]
async fn merge_keeps_tits_from_both_chats() {
    let (_container, db) = start_postgres().await;
    let chat = SplitChat::create(&db).await;
    ...
    chat.add_tits().await;
    // the trigger on Tits spends one bonus attempt per write, including the inserts above, so the
    ...
    let tits = chat.tits().await;
    assert_eq!(tits.len(), 2, "both users must keep a tit in the surviving chat");
    assert_eq!(tits[0].0, 3, "the tits of a user present in both chats must be summed");
    assert_eq!(tits[1].0, 7, "a tit present only in the merged-away chat must survive");
    // the trigger decrements bonus_attempts once per write, which the merge compensates for
    assert_eq!((tits[0].1, tits[1].1), (kept + moved, moved_only),
        "the merge must neither lose nor invent bonus attempts");

    assert_eq!(chat.tits_left().await, 0);
}
...
impl SplitChat {
    ...
    async fn add_tit_of_the_day(&self) {
        sqlx::query!("ALTER TABLE Titties_of_Day DISABLE TRIGGER trg_check_dod_timestamp")
            .execute(&self.db)
            .await.expect("couldn't mute the trigger");
        sqlx::query!("INSERT INTO Titties_of_Day (chat_id, winner_uid, created_at) VALUES ($1, $2, current_date - 1)",
                self.instance_row, UID)
            .execute(&self.db)
            .await.expect("couldn't create a tit of the day");
    ...
    async fn add_shrinks(&self) {
        sqlx::query!("INSERT INTO Stale_Tit_Shrinks (chat_id, uid, lost_length, created_at) VALUES ($1, $3, 5, current_date), ($2, $3, 3, current_date)",
    ...
    async fn add_tits(&self) {
        sqlx::query!("INSERT INTO Tits (uid, chat_id, length, bonus_attempts) VALUES ($1, $2, 1, 2), ($1, $3, 2, 3), ($4, $3, 7, 4)",
                UID, self.id_row, self.instance_row, UID + 1)
            .execute(&self.db)
            .await.expect("couldn't create tits");
    }
    ...
    async fn tit_of_the_day_days_ago(&self) -> i32 {
        sqlx::query_scalar!(r#"SELECT current_date - created_at AS "days_ago!" FROM Titties_of_Day WHERE chat_id = $1"#,
                self.id_row)
            .fetch_one(&self.db)
            .await.expect("the tit of the day must survive the merge")
    }

    async fn tits_of_the_day_left(&self) -> i64 {
        sqlx::query_scalar!(r#"SELECT count(*) AS "count!" FROM Titties_of_Day WHERE chat_id = $1"#, self.instance_row)
            .fetch_one(&self.db)
            .await.expect("couldn't count the orphaned tits of the day")
    }

    async fn lost_length(&self) -> i64 {
        sqlx::query_scalar!(r#"SELECT lost_length AS "lost_length!" FROM Stale_Tit_Shrinks WHERE chat_id = $1"#,
                self.id_row)
            .fetch_one(&self.db)
    ...
    async fn tits(&self) -> Vec<(i64, i32)> {
        sqlx::query!(r#"SELECT length AS "length: i64", bonus_attempts FROM Tits WHERE chat_id = $1 ORDER BY uid"#,
                self.id_row)
            .fetch_all(&self.db)
            .await.expect("couldn't fetch the tits")
            .iter()
    ...
    async fn bonus_attempts(&self, uid: i64, chat_row: i64) -> i32 {
        sqlx::query_scalar!("SELECT bonus_attempts FROM Tits WHERE uid = $1 AND chat_id = $2", uid, chat_row)
            .fetch_one(&self.db)
            .await.expect("the tit must exist")
    }

    async fn tits_left(&self) -> i64 {
        sqlx::query_scalar!(r#"SELECT count(*) AS "count!" FROM Tits WHERE chat_id = $1"#, self.instance_row)
            .fetch_one(&self.db)
            .await.expect("couldn't count the leftover tits")
    }
}

async fn clear_tits_and_chats(db: &Pool<Postgres>) {
    sqlx::query!("DELETE FROM Tits")
        .execute(db)
        .await.expect("couldn't delete tits");
...
async fn two_separate_chats(db: &Pool<Postgres>, chats: &repo::Chats, full: ChatIdFull) {
    ...
    let ids = sqlx::query_scalar!("INSERT INTO Chats (chat_id, chat_instance) VALUES ($1, NULL), (NULL, $2) RETURNING id",
        ...
        .await.expect("couldn't create chats");
    assert_eq!(ids.len(), 2);
    sqlx::query!("INSERT INTO Tits (uid, chat_id, length) VALUES ($1, $2, 1), ($1, $3, 2)",
            UID, ids[0], ids[1])
        .execute(db)
        .await.expect("couldn't create tits");

    let chat_id = full.to_partiality(Default::default());
    tits.create_or_grow(USER_ID, &chat_id, LengthChange::signed(0))
        .await
    ...
    let tit = tits.get_top(&chat_id_kind, Offset::new(0), Limit::literal(1), DaysCount::new(7))
        .await.expect("couldn't fetch the tit");
    assert_eq!(tit.len(), 1);
    assert_eq!(tit[0].length, 3);
}
