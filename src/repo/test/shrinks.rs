...
/// Inserts a tit directly (bypassing `create_or_grow`) with an explicit age, so we can seed
/// tits that look stale. A direct INSERT is required because the `Tits` BEFORE UPDATE trigger
/// forbids touching a row that was grown today — which a freshly created tit always is.
async fn seed_aged_tit(db: &Pool<Postgres>, internal_chat_id: i64, uid: i64, length: i64, days_ago: i32) {
    sqlx::query!(
        "INSERT INTO Tits (uid, chat_id, length, updated_at) \
            VALUES ($1, $2, $3, current_timestamp - make_interval(days => $4))",
        uid, internal_chat_id, length, days_ago)
        .execute(db)
        .await
        .expect("couldn't seed an aged tit");
}

async fn seed_aged_tit_with_bonus_attempts(
    db: &Pool<Postgres>,
    ...
    sqlx::query!(
        "INSERT INTO Tits (uid, chat_id, length, updated_at, bonus_attempts) \
            VALUES ($1, $2, $3, current_timestamp - make_interval(days => $4), $5)",
        uid, internal_chat_id, length, days_ago, bonus_attempts + 1)
        .execute(db)
        .await
        .expect("couldn't seed an aged tit with a bonus attempt");
}

async fn bonus_attempts_of(db: &Pool<Postgres>, uid: i64, internal_chat_id: i64) -> i32 {
    sqlx::query_scalar!("SELECT bonus_attempts FROM Tits WHERE uid = $1 AND chat_id = $2", uid, internal_chat_id)
        .fetch_one(db)
        .await
...
async fn logged_shrinks_count(db: &Pool<Postgres>, uid: i64) -> i64 {
    sqlx::query_scalar!("SELECT count(*) AS \"c!\" FROM Stale_Tit_Shrinks WHERE uid = $1", uid)
        .fetch_one(db)
        .await
...
async fn updated_at_of(db: &Pool<Postgres>, uid: i64, internal_chat_id: i64) -> chrono::DateTime<chrono::Utc> {
    sqlx::query_scalar!("SELECT updated_at FROM Tits WHERE uid = $1 AND chat_id = $2", uid, internal_chat_id)
        .fetch_one(db)
        .await
...
async fn length_of(db: &Pool<Postgres>, uid: i64, internal_chat_id: i64) -> i64 {
    sqlx::query_scalar!("SELECT length FROM Tits WHERE uid = $1 AND chat_id = $2", uid, internal_chat_id)
        .fetch_one(db)
        .await
...
async fn test_perform_daily_shrink() {
    ...
    // A fresh tit for USER_ID — also creates the Chats row. It must NOT be shrunk (grown today).
    users.create_or_update(USER_ID, NAME)
        .await.expect("couldn't create the primary user");
    tits.create_or_grow(USER_ID, &CHAT_ID_KIND.into(), LengthChange::signed(100))
        .await.expect("couldn't create the fresh tit");
    let chat_id = internal_chat_id(&db).await;

    // A stale, positive tit — the only expected victim.
    ...
    seed_aged_tit(&db, chat_id, victim_uid, 100, 10).await;
    let victim_updated_at_before = updated_at_of(&db, victim_uid, chat_id).await;

    // A stale but zero-length tit — must be left alone (only length > 0 shrinks).
    let zero_uid = UID + 2;
    users.create_or_update(UserId::literal(zero_uid), "zero-length")
        .await.expect("couldn't create the zero-length user");
    seed_aged_tit(&db, chat_id, zero_uid, 0, 10).await;

    // A stale, positive tit with a pending bonus attempt — shrinking it must not silently burn
    // the bonus attempt (the Tits trigger decrements it on every update unless counteracted).
    let bonus_uid = UID + 3;
    users.create_or_update(UserId::literal(bonus_uid), "has-bonus")
        .await.expect("couldn't create the bonus user");
    seed_aged_tit_with_bonus_attempts(&db, chat_id, bonus_uid, 100, 10, 1).await;
    ...
    assert_eq!(events.len(), 2, "the two stale, positive tits should have shrunk");
    ...
    // The fresh and the zero-length tits are untouched.
    assert_eq!(length_of(&db, UID, chat_id).await, 100, "the fresh tit must not shrink");
    assert_eq!(length_of(&db, zero_uid, chat_id).await, 0, "the zero-length tit must not shrink");
...
async fn test_perform_daily_shrink_ramps_up_the_ratio() {
    ...
    users.create_or_update(USER_ID, NAME)
        .await.expect("couldn't create the primary user");
    tits.create_or_grow(USER_ID, &CHAT_ID_KIND.into(), LengthChange::signed(1))
        .await.expect("couldn't create a dummy tit and the Chats row");
    let chat_id = internal_chat_id(&db).await;
    ...
    seed_aged_tit(&db, chat_id, just_overdue_uid, 1000, 7).await;
    ...
    seed_aged_tit(&db, chat_id, fully_ramped_uid, 1000, 10).await;

    // Way overdue: the ramp is capped, so this must lose the same as the fully-ramped tit above,
    // not more.
    ...
    seed_aged_tit(&db, chat_id, way_overdue_uid, 1000, 20).await;
...
/// The loss formula floors at `GREATEST(1, ...)`, so any neglected, positive-length tit loses at
/// least 1 cm per (real, daily-cadenced) run no matter how small `ratio * length` rounds down to —
/// it can always reach exactly 0 eventually, and `LEAST(length, ...)` caps the loss at the tit's
/// current length so it can never be shrunk past 0 into negative territory. This pins down the
/// terminal step directly: a 1 cm tit with a tiny ratio still loses exactly 1 cm, landing on 0.
#[tokio::test]
async fn test_perform_daily_shrink_floor_reaches_exactly_zero() {
    ...
    seed_aged_tit(&db, chat_id, victim_uid, 1, 100).await;
    ...
    let event = events.iter().find(|e| e.uid == UserId::literal(victim_uid))
        .expect("the 1cm tit must still shrink despite the tiny ratio");
...
/// "grown within N days" and shrinking *everyone*, including tits grown moments ago, with no
...
async fn test_perform_daily_shrink_rejects_overflowing_grace_days_instead_of_wrapping() {
    ...
    users.create_or_update(UserId::literal(victim_uid), "hundred-days-old")
        .await.expect("couldn't create the victim");
    seed_aged_tit(&db, chat_id, victim_uid, 100, 100).await;
...
async fn test_get_shrinks_for_date_only_returns_that_day() {
    ...
    assert_eq!(recent[0].length, 90, "the victim's current (post-shrink) length, joined from Tits");
}
...
async fn test_get_shrinks_for_date_pages() {
    ...
    tits.create_or_grow(USER_ID, &CHAT_ID_KIND.into(), LengthChange::signed(1))
        .await.expect("couldn't create a dummy tit");
    let chat_id = internal_chat_id(&db).await;
    ...
    for (n, lost) in [(1i32, 30i64), (2, 20), (3, 10)] {
        ...
        seed_aged_tit(&db, chat_id, uid, 100, 10).await;
...
async fn test_get_player_uids() {
    ...
    users.create_or_update(USER_ID, NAME)
        .await.expect("couldn't create the primary user");
    tits.create_or_grow(USER_ID, &CHAT_ID_KIND.into(), LengthChange::signed(1))
        .await.expect("couldn't create the first tit");
    let chat_id = internal_chat_id(&db).await;
    ...
    for n in 1..=2 {
        ...
        seed_aged_tit(&db, chat_id, uid, 10, 1).await;
    }

    let mut uids = tits.get_player_uids(&CHAT_ID_KIND).await
